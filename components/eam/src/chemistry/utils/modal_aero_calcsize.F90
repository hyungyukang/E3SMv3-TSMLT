...
!public modal_aero_calcsize_sub_e3sm
...
!  write(*,*) 'End of     modal_aero_calcsize_init'

end subroutine modal_aero_calcsize_init

!===============================================================================

!subroutine modal_aero_calcsize_sub_e3sm(state, deltat, pbuf, ptend, do_adjust_in, &
...
!
!   !-----------------------------------------------------------------------------------
!   !Extract info about optional variables and initialize local variables
!   !accordingly
!   !------------------------------------------------------------------------------------
!   !
!   !The default behavior is to update species mass mixing ratios (interstitial
!   !and
!   !cloud borne), but there are other calls (such as radiation diagnostics and
!   !otherwise)
!   !which might just require updated aerosol sizes without any update to species
!   !mmr
!   !-------------------------------------------------------------------------------------
!
!   update_mmr = .true. !update mmr for both interstitial and cloud borne
!aerosols
...
!           call endrun('list_idx_in is present but dgnumdry_m is
!missing'//errmsg(__FILE__,__LINE__))
!
!      if(.not. allocated(dgnumdry_m)) &
!           call endrun('list_idx_in is present but dgnumdry_m is not
!allocated'//errmsg(__FILE__,__LINE__))
!      dgncur_a => dgnumdry_m(:,:,:)
!
!      !update_mmr should be true ONLY for list_idx = 0
!      if(list_idx_local > 0 .and. update_mmr) then
!         call endrun('update_mmr should be FLASE for
!list_idx>0'//errmsg(__FILE__,__LINE__))
!      endif
!   else
...
!         write(err_msg,*)'Cannot adjust aerosol size if size adjustment is not
!allowed', &
!              ' do_adjust_allowed is:',do_adjust_allowed,' and do_adjust_in
!is:',do_adjust_in, &
...
!   endif
!
!   !For transerfering aerosols between modes (Aitkem<-->Accumulation) based on
!   !their new size
!   do_aitacc_transfer = do_aitacc_transfer_allowed(list_idx_local)
!   if (present(do_aitacc_transfer_in)) then
!      if(do_aitacc_transfer_in .and.
!.not.do_aitacc_transfer_allowed(list_idx_local)) then
!         write(err_msg,*)'Cannot transfer species between modes if transfer is
!not allowed for radiation', &
!              ' list:', list_idx_local,' do_aitacc_transfer_allowed
!is:',do_aitacc_transfer_allowed(list_idx_local), &
!              ' and do_aitacc_transfer_in is:',do_aitacc_transfer_in,';',
!errmsg(__FILE__,__LINE__)
!         call endrun(trim(err_msg))
!      endif
...
!         write(err_msg,*)'ptend should be present if update_mmr is true
!(update_mmr ', &
...
!   endif
!
!   !if(present(caller) .and. masterproc) write(iulog,*)'modal_aero_calcsize_sub
!   !has been called by ', trim(caller)
!
!   pdel     => state%pdel !Only required if update_mmr = .true.
!   state_q  => state%q    !BSINGH - it is okay to use state_q for num mmr but
!not for specie mmr (as diagnostic call may miss some species)
!
!   !----------------------------------------------------------------------------
...
!      !----------------------------------------------------------------------
!      !interstitial
!      call set_initial_sz_and_volumes(list_idx_local, top_lev, ncol, imode, &
!!input
!           dgncur_a, v2ncur_a, dryvol_a)
!!output
!
!      !cloud borne
!      call set_initial_sz_and_volumes(list_idx_local, top_lev, ncol, imode, &
!!input
!           dgncur_c, v2ncur_c, dryvol_c)
!!output
!
!      !----------------------------------------------------------------------
!      !Find # of species in this mode
!      !----------------------------------------------------------------------
!      !(for radiation diagnostics, # of species in a mode can be differnet from
!      !the default)
...
!      !       Algorithm:calculate aerosol diameter from mass, number, and fixed
!      !       sigmag
!      !
...
!      !----------------------------------------------------------------------
!      call compute_dry_volume(top_lev, ncol, imode, nspec, state, pbuf,
!dryvol_a, dryvol_c, list_idx_local)
!
!
!      ! do size adjustment based on computed dry diameter values and update the
!      ! diameters
!      call size_adjustment(list_idx_local, top_lev, ncol, lchnk, imode,
!dryvol_a, state_q, & !input
!           dryvol_c, pdel, do_adjust, update_mmr, do_aitacc_transfer, deltatinv,
!fracadj, pbuf,  & !input
!           dgncur_a, dgncur_c, v2ncur_a, v2ncur_c,
!& !output
!           drv_a_accsv, drv_c_accsv, drv_a_aitsv, drv_c_aitsv, drv_a_sv,
!drv_c_sv,               & !output
!           num_a_accsv, num_c_accsv, num_a_aitsv, num_c_aitsv, num_a_sv,
!num_c_sv,               & !output
...
!      call aitken_accum_exchange(ncol, lchnk, list_idx_local, update_mmr,
!tadjinv, &
...
!      call update_cld_brn_mmr(list_idx_local, top_lev, ncol, lchnk, pcnst,
!deltat, pbuf, dotendqqcw, dqqcwdt)
!
!      !----------------------------------------------------------------------
!      ! do outfld calls
!      !----------------------------------------------------------------------
...
!      if ( .not. do_aitacc_transfer) return ! return if transfer of species not
!required
!      ilist = 0 ! write output only for list_idx_local = 0
!      do iq = 1, nspecfrm_csizxf(ilist)
!
!         ! aer_type=1 does interstitial ("_a"); aer_type=2 does activated
!         ! ("_c");
!
...
!
!            fieldname = trim(tmpnamea) // '_sfcsiz3'
!            call outfld( fieldname, qsrflx(:,lsfrm,3,cld_brn_aero), pcols,
!lchnk)
!
!            fieldname = trim(tmpnameb) // '_sfcsiz3'
!            call outfld( fieldname, qsrflx(:,lstoo,3,cld_brn_aero), pcols,
!lchnk)
!
!            fieldname = trim(tmpnamea) // '_sfcsiz4'
!            call outfld( fieldname, qsrflx(:,lsfrm,4,cld_brn_aero), pcols,
!lchnk)
!
!            fieldname = trim(tmpnameb) // '_sfcsiz4'
!            call outfld( fieldname, qsrflx(:,lstoo,4,cld_brn_aero), pcols,
!lchnk)
!         endif
!      end do   ! iq = ...
!
...
!
!return
!end subroutine modal_aero_calcsize_sub_e3sm
...
!  print*, 'HGCALL in enter size_sub'
...
!              print*, 'HGDRYVOLA in ze_sub', dryvol_a(i,k), q(i,k,la)
            end do
         end do
...
!              print*, 'HGDRYVOLC in ze_sub', dryvol_a(i,k),fldcw(i,k)
            end do
         end do
      end do
