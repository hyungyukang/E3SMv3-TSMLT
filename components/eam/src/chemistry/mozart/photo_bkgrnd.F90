...
contains
  ...
  subroutine photo_bkgrnd_calc(f107, o_den, o2_den, n2_den, no_den, zint, rates, & 
                               qbko1_out, qbko2_out, qbkn2_out, qbkn1_out, qbkno_out,do_jshort)
   ...
   ! arguments
    ...
    if ( .not. present(do_jshort)) do_jshort=.false.
    ...
    if ((.not.do_jshort) .or. (ptop_ref < 10._r8)) then 
       ...
       zmaj(1,:) = o_den(2:pverp) 
       zmaj(2,:) = o2_den(2:pverp) 
       zmaj(3,:) = n2_den(2:pverp) 
       zno(:)    = no_den(2:pverp)
    end if
    ...
    if ((.not.do_jshort) .or. (ptop_ref < 10._r8)) then 
       ...
       if (jo_ndx>0)  rates(:,jo_ndx)  = rates(:,jo_ndx)  + qbko1(:)/o_den(2:pverp)
       if (jo2_ndx>0) rates(:,jo2_ndx) = rates(:,jo2_ndx) + qbko2(:)/o2_den(2:pverp)
       if (jn2_ndx>0) rates(:,jn2_ndx) = rates(:,jn2_ndx) + qbkn2(:)/n2_den(2:pverp)
       if (jn_ndx >0) rates(:,jn_ndx)  = rates(:,jn_ndx)  + qbkn1(:)/n2_den(2:pverp)
       if (jno_ndx>0) rates(:,jno_ndx) = rates(:,jno_ndx) + qbkno(:)/no_den(2:pverp)
    endif
