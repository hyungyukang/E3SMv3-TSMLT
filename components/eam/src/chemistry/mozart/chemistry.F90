...
!================================================================================================
  ...
  subroutine chem_readnl(nlfile)
    ...
    namelist /chem_inparm/ chem_freq, airpl_emis_file, &
         ...
         lght_no_prd_factor, &
         depvel_file,depvel_lnd_file, drydep_srf_file, &
    ...
    if (masterproc) then
      write(iulog,*)'after tracer_cnst_defaultopts -----------'
    endif
    ...
    if (masterproc) then
      write(iulog,*)' before tracer_srcs_defaultopts -----------'
    endif
    ...
    if (masterproc) then
       ...
       print*, "AFTER READNL", srf_emis_specifier
    end if

!   if (masterproc) then
!      call endrun('chem_readnl: PASS reading namelist without chem_use_chemtrop')
!   end if
...
!   if (masterproc) write(iulog,*)'before flbc_chk'
    ...
    !-----------------------------------------------------------------------
!   if (masterproc) then
!     write(iulog,*)'before tracer_cnst_adv'
!   endif
    ...
    !-----------------------------------------------------------------------
!   if (masterproc) then
!     write(iulog,*)'before tracer_srcs_adv'
!   endif

    call tracer_srcs_adv(pbuf2d, phys_state)

!   if (masterproc) then
!     write(iulog,*)'after  tracer_srcs_adv'
!   endif
...
!   do k = 1,pver
!   do i = 1,ncol
!      print*, 'HGCHEM 1',ptend%q(i,k,138),ptend%q(i,k,1), ptend%q(i,k,137),ptend%q(i,k,139)
!   end do
!   end do
...
!   do k = 1,pver
!   do i = 1,ncol
!      print*, 'HGCHEM 2',ptend%q(i,k,138),ptend%q(i,k,1), ptend%q(i,k,137),ptend%q(i,k,139)
!   end do
!   end do
!-----------------------------------------------------------------------
! get current calendar day of year
...
!   do k = 1,pver
!   do i = 1,ncol
!      print*, 'HGCHEM 3',ptend%q(i,k,138),ptend%q(i,k,1), ptend%q(i,k,137),ptend%q(i,k,139)
!   end do
!   end do
!-----------------------------------------------------------------------
