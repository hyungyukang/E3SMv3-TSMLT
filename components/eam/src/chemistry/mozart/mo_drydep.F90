...
contains
  ...
  subroutine drydep_update( state, cam_in )
    ...
    !print*, 'HGFFFF', nddvels,shape(lnd(state%lchnk)%dvel),cam_in%depvel
!   print*, 'HGFFFF', nddvels,shape(cam_in%depvel)
...
! HGKANG ---
  !---------------------------------------------------------------------------
  subroutine dvel_inti_table( depvel_file, table )
    !---------------------------------------------------------------------------
  ...
  subroutine dvel_inti_xactive( depvel_lnd_file )
    ...
    if( masterproc ) then
       write(iulog,*) 'drydep_inti: following species have dry deposition',nddvels
       do i=1,nddvels
          if( len_trim(drydep_list(i)) > 0 ) then
             write(iulog,*) 'drydep_inti: '//trim(drydep_list(i))//' is requested to have dry dep'
          endif
